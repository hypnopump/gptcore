...
# supports Umat, not Wmat
from .rwkv_triton_v6hypno_chunked import chunk_rwkv6hypno
...
# supports Umat ^ Zmat ^ Wmat
from .rwkv_triton_v7wuz import fused_recurrent_rwkv7hypno2
...
chunk_rwkv6hypno_compiled = torch._dynamo.disable(torch.jit.ignore(chunk_rwkv6hypno))
...
class RWKV6_0_AttentionSubLayer(model.core.TransformerLayerPart, model.interface.IAttentionSubLayer):
    ...
    def forward(self, xq : Tensor, xk : Tensor, xv : Tensor, recurrent_memory : Optional[Tensor] = None):
        ...
        time_decay = self.time_decay.to(r) # .float()  # (KVH,K)
        time_first = self.time_first.to(r) # float()  # (KVH,K)
        if KVH < H:
            reps = H // KVH
