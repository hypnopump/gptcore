...
# @torch.compile
def rwkv_inner_umat(r,k,v,w,u,kv_state,chunk_len:int=24,precision_dtype:torch.dtype=torch.float32):
    """
    expects
    r : (B,H,L,K)
    k : (B,H,L,K)
    ...
    if L == 1:
        ...
        pre_out = torch.einsum('bhic,hcd,bhic,bhid->bhid', r, u, k, v)
        # chunked view of r, k, v
        r = r.view(B,H,N,T,K)
        k = k.view(B,H,N,T,K)
        v = v.view(B,H,N,T,V)
